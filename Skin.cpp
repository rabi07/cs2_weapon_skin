...
inline int64_t to_steam64(int id)
{
	int account_type = id % 2;
	int account_id = (id - account_type) / 2;
	return 76561197960265728L + (account_id * 2) + account_type;
}

void CEntityListener::OnEntitySpawned(CEntityInstance* pEntity)
{
	CCSWeaponBase* pBasePlayerWeapon = dynamic_cast<CCSWeaponBase*>(pEntity);
	if(!pBasePlayerWeapon)return;
	if(!pBasePlayerWeapon->m_AttributeManager().m_Item().m_iAccountID())return;
	
	int64_t steamid = to_steam64(pBasePlayerWeapon->m_AttributeManager().m_Item().m_iAccountID());
